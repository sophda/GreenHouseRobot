...
from utils_.utils import (centernet_correct_boxes, decode_bbox, letterbox_image,
                         nms)
...
class CenterNet(object):
    _defaults = {              ####Epoch1200-Total_Loss1.4515-Val_Loss1.8970.pth
        "model_path"        : 'D:\PY code\socket\centernet\centernet.pth', #'model_data/centernet_resnet50_voc.pth',Epoch1000-Total_Loss1.6163-Val_Loss0.9350
        # "classes_path"      : 'model_data/voc_classes.txt',D:\huihuipython\centernet-pytorch\logs\Epoch1200-Total_Loss1.4515-Val_Loss1.8970.pth
        "classes_path"      :'D:\PY code\socket\centernet\model_data/voc_classes.txt',
        # "model_path"        : 'model_data/centernet_hourglass_coco.h5',
        # "classes_path"      : 'model_data/coco_classes.txt',
        "backbone"          : "resnet50",
        "image_size"        : [2240,2240,3],
        "confidence"        : 0.3,
    ...
    def detect_image(self, image):
        ...
        for i, c in enumerate(top_label_indices):
            ...
            for j in range(thickness):
                ...
                #print("数字：",left,top,right,bottom)
                cx = int((left+right)/2)
                cy = int((top+bottom)/2)
                #cv2.circle(image,(cx,cy),5,(0,0,255))
                draw.ellipse((cx-10, cy-10, cx + 10, cy + 10), fill=(255, 255, 0))
                center_cor.append(np.array([cx, cy]))
            ...
            cx1 = int((left + right) / 2)
            cy1 = int((top + bottom) / 2)
            listx.append(cx1)
            listy.append(cy1)

            # depth_list.append(dep[cy1, cx1])
            #print(i)
            #draw.text(text_origin, str('num:{0}({1},{2})'.format(i + 1, cx1, cy1)), fill=(0, 0, 0), font=font)
            del draw
        center_cor = np.array(center_cor)
        np.save('center.npy', center_cor)
        return image, depth_list,len(top_label_indices)*2  # listx, listy
